...
func create_array_to_send() -> Array:
	...
	for class_id_int in class_dictionary.keys():
		compress_class_objects(class_id_int)
...
func compress_class_objects(class_id_int : int) -> void:
	bit_packer.compress_class_id(class_id_int)
	bit_packer.compress_int_into_x_bits(class_dictionary[class_id_int].size(), BitStream.BITS_FOR_N_OBJECTS)
