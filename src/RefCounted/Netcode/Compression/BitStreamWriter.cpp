...
void BitStreamWriter::write_int(BitStream *const bitstream, int inData, int inBitCount)
{
...
void BitStreamWriter::write_scratch_to_buffer(BitStream *const bitstream)
{
...
PackedByteArray BitStreamWriter::get_byte_array(BitStream *const bitstream)
{
...
void BitStreamWriter::flush_scratch_to_buffer(BitStream *const bitstream)
{
...
void BitStreamWriter::compress_int_into_x_bits(BitStream *const bitstream, int inData, int inBitCount, bool is_signed)
{
	if (is_signed)
	{
...
void BitStreamWriter::compress_int_array(BitStream *const bitstream, Array int_array, bool is_signed)
{
...
void BitStreamWriter::variable_compress(BitStream *const bitstream, Variant value, bool is_signed)
{
	int n_bits;
...
void BitStreamWriter::compress_vector_into_x_bits(BitStream *const bitstream, Vector2 vec, int n_bits, bool is_signed)
{
	assert(n_bits % 2 == 0);
...
void BitStreamWriter::compress_float_into_x_bits(BitStream *const bitstream, float in_float, int n_bits, bool is_signed, int n_decimals)
{
	int float_to_int = int(round(in_float * pow(10.0, n_decimals)));
	compress_int_into_x_bits(bitstream, float_to_int, n_bits, is_signed);
}

void BitStreamWriter::compress_bool(BitStream *const bitstream, bool bool_var)
{
	write_int(bitstream, int(bool_var), 1);
}

void BitStreamWriter::compress_unit_vector(BitStream *const bitstream, Vector2 invec)
{
...
void BitStreamWriter::compress_instance_id(BitStream *const bitstream, int instance_id)
{
	write_int(bitstream, instance_id, BitStream::N_CLASS_INSTANCE_BITS);
}

void BitStreamWriter::compress_class_id(BitStream *const bitstream, int class_id)
{
	assert(class_id <= 878500);
	write_int(bitstream, class_id, BitStream::N_CLASS_ID_BITS);
}

void BitStreamWriter::compress_frame(BitStream *const bitstream, int frame)
{
	write_int(bitstream, frame, BitStream::BITS_FOR_FRAME);
}

void BitStreamWriter::compress_quantized_input_vec(BitStream *const bit_stream, Vector2 input_vec)
{
